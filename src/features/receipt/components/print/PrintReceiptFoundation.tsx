...
export function PrintReceiptFoundation({ data }: Readonly<{ data: ReceiptData }>) {
  ...
  <div className="grid grid-cols-2 gap-4 text-[10px] text-muted-foreground pt-2">
      <div className="space-y-0.5">
        <div className="font-extrabold text-foreground uppercase tracking-wider text-xs break-words hyphens-none">
          {data.foundationName}
        </div>
        <div className="break-words hyphens-none">{data.foundationAddress}</div>
      ...
      <div className="text-right space-y-0.5 border-l border-border pl-4">
        ...
        <div className="break-words hyphens-none">a.n. {data.bankAccountName}</div>
      </div>
    </div>
  );
}
